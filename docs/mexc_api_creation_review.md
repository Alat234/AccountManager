...
## Уточнення передумови 2FA — 2026-09-11

Поточна поведінка після виправлення 2026-09-10:

- Немає збереженого two_fa_secret: запропонувати Link 2FA, потім Create API.
- Секрет є, twofa_pending відсутній/false: використати збережений секрет.
- Секрет є, twofa_pending=true: запитати, чи користувач уже завершив прив’язку
  на MEXC і чи збережений секрет відповідає активній 2FA. Підтвердження очищує
  прапорець і продовжує Create API. Відмова зупиняє дію без повторної прив’язки.

Раннє закриття браузера може залишити результат непідтвердженим навіть після
успішної прив’язки на MEXC. Наявність прапорця не означає, що 2FA відсутня.
Підтвердження користувача не є автоматичною перевіркою секрету сервером;
MEXC усе одно перевіряє код під час створення API.

Актуальна збірка: `dist/twofa-background-20260910/main.exe`.
Деталі та межі перевірки: [огляд 2FA](mexc_2fa_link_review.md).
